...
- The **Advanced** dashboard (key grid, mock devices, brightness, image-fit modes,
  real-time log feed) is a developer view and is **not** part of the released binaries.
  Build it from source with `mise run s-advanced`, then switch the web UI to **Advanced**
  (top-right).
