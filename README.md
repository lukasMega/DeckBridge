...
> NOTE: For professional usage, use officialy supported Network Dock + Stream Deck devices

This tool is an alternative to a "Network Dock" device (>60 USD).
Bridges a USB stream deck to Elgato Stream Deck software over TCP/CORA.

Supported devices: **Mirabox 293V3/Ajazz**, **Mirabox 293S**, **Mirabox K1 Pro**, **Stream Deck MK.2**, **Stream Deck Mini**.

**Tested on macOS hardware:** Mirabox 293V3, Mirabox 293S, Mirabox K1 Pro, and Elgato Stream Deck Mini. Stream Deck MK.2 and the Linux/Windows builds are implemented but **not hardware-verified**.
...
Both real drivers — `ElgatoHidDriver.open()` ([hid-driver-base.ts](ts/src/devices/hid-driver-base.ts)) and `MiraboxDriver.open()` ([mirabox.ts](ts/src/mirabox.ts)) — use the same two-step open:

1. **Path-based open** — if the model defines `usagePage` + `usage`, calls `findHidPath()` → `deckbridge-native` Rust cdylib → `mirabox_hid_find_path(vid, usagePage, usage)`. Opening by path avoids claiming system-owned interfaces on macOS (the OS typically grants the first `hid_open` caller exclusive access to a VID+PID).
2. **VID+PID fallback** — calls `hid_open(VID, PID)` for each PID in `model.usbProductIds` until one succeeds.

Only the Mirabox models set `usagePage`/`usage` (all use `0xffa0`/`1`); Elgato models skip step 1.

### libhidapi loading

`loadHidapi()` ([ffi/hidapi.ts](ts/src/ffi/hidapi.ts)) tries a platform-specific candidate list via `FFI.dlopen`:

| Platform | Candidates (tried in order) |
|----------|-----------------------------|
| macOS | `/opt/homebrew/lib/libhidapi.dylib` (Apple Silicon), `/usr/local/lib/libhidapi.dylib` (Intel) |
| Linux | `/usr/lib/x86_64-linux-gnu/libhidapi-hidraw.so.0`, `/usr/lib/libhidapi-hidraw.so.0` |
...
```c
int mirabox_hid_find_path(uint16_t vid, uint16_t usage_page, uint16_t usage,
                          uint8_t *buf, size_t buf_len);
// Returns 1 and writes the null-terminated HID path into buf on success, 0 if not found.
```
...
flowchart LR
    ...
    BUNDLE["ts/dist/bundle.js<br/>~776 kB ESM<br/>(worker string + native dylibs inlined)"]
...
mise run ci-checks   # lint + typecheck + test
```
...
| `image-cache.test.ts` | `hashJpeg` determinism (incl. sampled hashing for buffers >8KB: length + first/last 4KB), `LruCache` eviction/recency, `makeCacheKey` namespacing |
...
│   ├── tsconfig.json
│   ├── package.json    ← dev deps: esbuild, typescript, buffer, eventemitter3
...
│   │   ├── image-cache.ts         ← LRU cache (sampled FNV-1a: length + first/last 4KB) to skip redundant FFI calls
...
│   └── dist/
│       ├── bundle.js              ← esbuild output (~776 kB, worker + native dylibs inlined)
