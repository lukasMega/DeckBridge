...
impl<W: JfifWrite> Encoder<W> {
    ...
    fn encode_image_internal<I: ImageBuffer, OP: Operations>(
        ...
        {
            // FORK (deckbridge): emit a single interleaved scan with optimized
            // huffman tables instead of one scan per component. Some hardware
            // decoders only support a single interleaved scan.
            self.encode_image_interleaved_optimized::<_, OP>(image, &q_tables)?;
    ...
    /// FORK (deckbridge): Encode all components with one interleaved scan
    /// using huffman tables optimized for this image.
    ///
    /// Two pass version of [Self::encode_image_interleaved]: pass 1 collects
    /// the quantized blocks in interleaved scan order and tallies the symbol
    /// frequencies (DC diff prediction follows the interleaved traversal, so
    /// the statistics match exactly what is written), pass 2 writes the scan
    /// with the optimized tables. Restart intervals are not supported here;
    /// callers with a restart interval fall back to the sequential path.
    fn encode_image_interleaved_optimized<I: ImageBuffer, OP: Operations>(
        &mut self,
        image: I,
        q_tables: &[QuantizationTable; 2],
    ) -> Result<(), EncodingError> {
        ...
        let blocks_per_mcu: usize = self
            .components
            .iter()
            .map(|c| {
                c.horizontal_sampling_factor as usize * c.vertical_sampling_factor as usize
            })
            .sum();

        // Pass 1: collect quantized blocks in interleaved scan order
        let mut blocks: Vec<(usize, [i16; 64])> =
            Vec::with_capacity(num_cols * num_rows * blocks_per_mcu);
        ...
        self.optimize_huffman_table_interleaved(&blocks);

        self.write_frame_header(&image, q_tables)?;
        self.writer
            .write_scan_header(&self.components.iter().collect::<Vec<_>>(), None)?;

        // Pass 2: write the blocks with the optimized tables
        let mut prev_dc = [0i16; 4];

        for &(i, ref q_block) in &blocks {
    ...
    /// FORK (deckbridge): Create new huffman tables optimized for blocks in
    /// interleaved scan order (see [Self::optimize_huffman_table] for the
    /// sequential equivalent)
    fn optimize_huffman_table_interleaved(&mut self, blocks: &[(usize, [i16; 64])]) {
        ...
        for table in 0..max_tables {
            ...
            for &(i, ref block) in blocks {
                let component = &self.components[i];

                if component.dc_huffman_table == table {
                    had_dc = true;

                    let diff = block[0] - prev_dc[i];
                    dc_freq[get_num_bits(diff) as usize] += 1;
                }
