// Shared fixtures for the image-path test suites (translator, image-render,
// image-pipeline).
//
// NOTE: intentionally NOT named `*.test.ts` so scripts/run-tests.mjs (which globs
// `test/*.test.ts`) never runs it as a standalone test — it is only imported by
// the real test files and bundled in via esbuild.
import type { DeviceModel } from '../../src/devices/driver.js';

// A valid 16×16 solid-red JPEG (generated via @julusian/image-rs, verified
// decodable by the Rust image-proc sidecar).
export const SOLID_RED_16X16_JPEG = Buffer.from([
  ,
]);

/** Minimal gen2-style DeviceModel with a passthrough JPEG transform and empty
 *  keyMap (identity key mapping). Only enough to satisfy the driver shape —
 *  the thin handler never reads model.image, so those values are placeholders. */
export function makePassthroughModel(): DeviceModel {
  return {
    id: 'test-passthrough',
    vendor: 'elgato',
    protocol: 'elgato-gen2',
    name: 'Test Passthrough Device',
    usbVendorId: 0x0fd9,
    usbProductIds: [0x00a5],
    keyCount: 15,
    columns: 5,
    rows: 3,
    keyWidth: 96,
    keyHeight: 96,
    image: {
      format: 'jpeg',
      width: 96,
      height: 96,
      rotate: 0,
      flipH: false,
      flipV: false,
      colorMode: 'rgb',
      maxBytes: 0,
      quality: 0.9,
      transform: 'passthrough',
    },
    wire: { packetSize: 1024, inSize: 512 },
    keyMap: {},
    cora: {
      productId: 0x00a5,
      usePhysicalIdentity: true,
    },
    driverKind: 'elgato-hid',
  };
}
