...
// Minimal 1×1 black JPEG (upscaled to the key size by firmware). Module scope, so
// clearKey does not rebuild 233 bytes per call on the USB worker thread.
const TINY_BLACK_JPEG = new Uint8Array([
  ...
  0x08, 0x01, 0x01, 0x00, 0x00, 0x3f, 0x00, 0xfb, 0x00, 0xff, 0xd9,
]);

/** All-black key image; the firmware upscales the 1×1 JPEG to the key size. */
export function gen2BlankImage(): Uint8Array {
  return TINY_BLACK_JPEG;
}

/** serial: report 0x06 from byte 2; firmware: report 0x05 from byte 6. Both
 *  carry their length in byte 1. */
export const GEN2_INFO_REPORTS = {
  serial: { reportId: 0x06, offset: 2, lengthPrefixed: true },
  firmware: { reportId: 0x05, offset: 6, lengthPrefixed: true },
};
