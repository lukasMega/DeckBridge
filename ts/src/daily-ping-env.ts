// May this process send the daily ping at all? (daily-ping.ts owns the payload.)
 ...
 *  (daily-ping.ts), so a suppressed ping gets retried same day, not next. */
...
export function suppressReason(input: SuppressInput): SuppressReason {
  ...
  if (
    isSet(env, 'DECKBRIDGE_NO_DAILY_PING') ||
    isSet(env, 'DECKBRIDGE_NO_TELEMETRY') ||
